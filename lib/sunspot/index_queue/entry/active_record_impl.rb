      ...
      class ActiveRecordImpl < ActiveRecord::Base
        ...
        class << self
          ...
          def add(klass, id, delete, priority)
            ...
            # Do not update if entry already exists
            if (!where(queue_entry_key).first)
              queue_entry = new(queue_entry_key.merge(:priority => priority))
        ...
        def too_many_attempts?
          attempts > ActiveRecordImpl.max_attempts
        end

        def deadly_error?
          ActiveRecordImpl.deadly_errors.any? { |e| error.to_s.include? e }
        end

        def deletable?
          ! ActiveRecordImpl.undeletable_classes.include?(record_class_name)
        end
